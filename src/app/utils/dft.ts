import {Block, BlockPoint, BlockSet, BlockType, Bounds} from "../models";
import dft from "../../assets/blocks/opv/dft.json";
...
function getDFT(blockList: Block[]) {
  let start = 0;
  let middle = 0;
  let end = 0;
  ...
  return dft.find(c => c['DBA_Name'] === name);
}

export function dftX(blockList: Block[]) {
  const entry = getDFT((blockList));
...
export function dftY(blockList: Block[]) {
  const entry = getDFT((blockList));
...
export function enumerateAll(): BlockPoint[] {
...
export function getBounds(points: BlockPoint[]): Bounds {
